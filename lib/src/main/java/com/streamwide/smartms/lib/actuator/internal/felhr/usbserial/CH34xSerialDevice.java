 ...
 *
 * @lastModifiedOn mar., 2 mars 2021 10:01:22 +0100
 */
...
public class CH34xSerialDevice extends UsbSerialDevice
{
    ...
    private static final String CLASS_NAME = "CH34xSerialDevice";
    ...
    public CH34xSerialDevice(@NonNull UsbDevice device, @Nullable UsbDeviceConnection connection, int iface)
    {
    ...
    public void setBaudRate(int baudRate)
    {
        if(baudRate <= 300)
        {
            ...
            if(ret == -1)
                Logger.error(CLASS_NAME, "SetBaudRate failed!");
    ...
    private int init()
    {
        ...
        {
            Logger.error(CLASS_NAME, "init failed! #1");
            return -1;
        }

        if(setControlCommandOut(0xa4, 0xdf, 0, null) < 0)
        {
            Logger.error(CLASS_NAME, "init failed! #2");
            return -1;
        }

        if(setControlCommandOut(0xa4, 0x9f, 0, null) < 0)
        {
            Logger.error(CLASS_NAME, "init failed! #3");
            return -1;
        }
        ...
        {
            Logger.error(CLASS_NAME, "init failed! #5");
            return -1;
        }

        if(setControlCommandOut(0x9a, 0x1312, 0xb282, null) < 0)
        {
            Logger.error(CLASS_NAME, "init failed! #6");
            return -1;
        }

        if(setControlCommandOut(0x9a, 0x0f2c, 0x0008, null) < 0)
        {
            Logger.error(CLASS_NAME, "init failed! #7");
            return -1;
        }

        if(setControlCommandOut(0x9a, 0x2518, 0x00c3, null) < 0)
        {
            Logger.error(CLASS_NAME, "init failed! #8");
            return -1;
        }
        ...
        if(setControlCommandOut(0x9a, 0x2727, 0x0000, null) < 0)
        {
            Logger.error(CLASS_NAME, "init failed! #10");
            return -1;
        }
    ...
    private int checkState(String msg, int request, int value, int[] expected)
    {
        ...
        {
            Logger.debug(CLASS_NAME, ("Expected " + expected.length + " bytes, but get " + ret + " [" + msg + "]"));
            return -1;
        }else
        {
            return 0;
    ...
    boolean checkCTS()
    {
        ...
        if(ret != 2)
        {
            Logger.debug(CLASS_NAME, ("Expected " + "2" + " bytes, but get " + ret));
            return false;
        }
    ...
    private int writeHandshakeByte()
    {
        ...
        {
            Logger.error(CLASS_NAME, "Failed to set handshake byte");
            return -1;
        }
        return 0;
    }
    ...
    boolean isRtsCtsEnabled() {
        return rtsCtsEnabled;
    }

    boolean isDtrDsrEnabled() {
        return dtrDsrEnabled;
    }

    void onCtsChanged(boolean cts){
        if(ctsState != cts)
        {
            ctsState = !ctsState;
            if (ctsCallback != null)
