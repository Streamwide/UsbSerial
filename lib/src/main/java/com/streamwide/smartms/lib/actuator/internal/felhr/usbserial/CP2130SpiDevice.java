 ...
 *
 * @createdBy  AndroidTeam on ven., 12 mars 2021 11:41:36 +0100
 ...
 *
 * @lastModifiedOn ven., 12 mars 2021 11:14:28 +0100
 */
...
public class CP2130SpiDevice extends UsbSpiDevice
{
    ...
    private static final String CLASS_NAME = "CP2130SpiDevice";
    ...
    public CP2130SpiDevice(@NonNull UsbDevice device, @Nullable UsbDeviceConnection connection, int iface)
    {
    ...
    @Override
    public void writeMOSI(@NonNull byte[] buffer)
    {
    ...
    @Override
    public void writeRead(@NonNull byte[] buffer, int lengthRead)
    {
    ...
    public void selectSlave(int nSlave)
    {
        ...
        {
            Logger.error(CLASS_NAME, "selected slave must be in 0-10 range");
            return;
        }
    ...
    private void setSetSpiWord(int channel, int freq)
    {
        ...
        {
            Logger.error(CLASS_NAME, "Channel not valid");
            return;
        }
        payload[1] = (byte) (freq);
        payload[1] = (byte) (payload[1] & 0xff | (1 << 3)); // Push pull chip select pin mode
