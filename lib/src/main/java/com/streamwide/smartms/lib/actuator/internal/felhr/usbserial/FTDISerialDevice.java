 ...
 *
 * @createdBy  AndroidTeam on ven., 12 mars 2021 14:18:19 +0100
 ...
 *
 * @lastModifiedOn ven., 12 mars 2021 14:16:56 +0100
 */
...
public class FTDISerialDevice extends UsbSerialDevice
{
    ...
    private static final String CLASS_NAME = "FTDISerialDevice";
    ...
    public FTDISerialDevice(@NonNull UsbDevice device, @NonNull UsbDeviceConnection connection)
    {
        this(device, connection, -1);
    }

    public FTDISerialDevice(@NonNull UsbDevice device, @NonNull UsbDeviceConnection connection, int iface)
    {
    ...
    static void copyData(byte[] src, byte[] dst)
    {
    ...
    public class FTDIUtilities
    {
        // Special treatment needed to FTDI devices
        @NonNull
        public byte[] adaptArray(@NonNull byte[] ftdiData)
        {
            int length = ftdiData.length;
            if(length > 64)
            {
        ...
        public void checkModemStatus(@NonNull byte[] data)
        {
            ...
            FTDISerialDevice.this.checkModemStatus(data,cts,dsr);

        }
    }

    void checkModemStatus(@NonNull byte[] data, boolean cts, boolean dsr){
    ...
    private short getBcdDevice() {
        ...
        return (short) ((descriptors[12] << 8) + (descriptors[13] & 0xff));
    }
